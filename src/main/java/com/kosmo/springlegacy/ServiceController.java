...
/*
 ...
 컨트롤러 클래스 역할르 부여하기 위한 어노테이션으로, 스프링 컨테이너가
 시작될 때 자동으로 빈이 생성된다.  
 */
@Controller
...
사용할 수 있다. 또한 미리 생성돼 있어야하고, 그렇지 않으면 서버가
정상적으로 실행되지 않는다. 
	 */
	MyService myService;
	...
	/*
	 서비스 객체의 호출을 위해 빈을 자동 주입 받는다. 
	 일반적으로 setter 메서드 없이 멤버 변수에 직접 어노테이션을 부착한다.
	...
	/*
	 사용자가 HTML페이지에서 각 링크를 클릭할 때 전달하는 userLottoNum파라미터를
	 lottoVO객체가 받아서 사용한다. 
	 */
