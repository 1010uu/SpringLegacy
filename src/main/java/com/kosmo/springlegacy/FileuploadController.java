...
public class FileuploadController {
	...
	//request, response 내장 객체를 사용하기 위해 매개 변수로 선언
	public void uploadPath(HttpServletRequest req,
		...
		//upload디렉토리는 정적 파일을 저장하기 위한 resources하위에 생성한다.
		
		//response내장 객체를 통해 MIME타입을 설정한다. 
		resp.setContentType("text/html; charset=utf-8");
		//View를 호출하지 않고 컨트롤러에서 즉시 출력
	...
	public String uploadList(HttpServletRequest req, Model model) {
		...
		for(File f : fileArray) {
			...
			fileMap.put(f.getName(), (int)Math.ceil(f.length()/1024.0)); //kb로 출력
		}
