...
public class FormController {
	...
	/*
	 폼 값 받기1] 
	 	파라미터로 전달된 값을 HttpServletRequest 내장 객체를
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		//request내장 객체의 getParameter()를 사용
		...
		ViewResolver가 전체경로를 조립하여 웹 브라우저에 출력한다.*/
		return "01Form/servletRequest";
	}
	
	/*
	 폼 값 받기2]@RequestParam어노테이션으로 폼 값 받기
	...
	public String joinMember(Model model,
		...
		//getParameter()로 받는 대신 어노테이션으로 받은 후 변수에 저장
	...
	/*
	 폼 값 받기3] 커맨드 객체를 이용해서 폼 값 한번에 받기
	 	조건1 : 쿼리 스트링으로 전달되는 파라미터의 갯수와 폼 값을 
	 		저장할 객체(DTO 혹은 VO)의 멤버변수의 갯수가 동일할 것을 권장.
	 	조건2 : 커맨드 객체를 사용할 때는 클래스명 앞글자를 소문자로 
	 		바꾼 형태의 매개변수를 사용해야한다.
	 		ex) MemberDTO => memberDTO
	 	조건3 : 커맨드 객체를 만들 때 파라미터의 갯수는 달라도 상관없으나
	 		파라미터명과 멤버변수명은 동일해야하고 이에 해당하는 getter/setter
	 		가 반드시 생성돼야 한다.
	 */
	...
	/*
	 폼 값 받기4] @PathVariable 어노테이션 사용하기
	 	...
	 	웹 브라우저는 요청명을 경로로 인식하므로 리소스를 사용할 때 경로를
	 	주의해야한다. 또한 파라미터의 갯수가 달라지면 404에러가 발생한다.
	 */
