...
public class ValidateController {
	...
	/*
	 메서드의 첫번째 파라미터
	  	...
	  	모댈 객체에 저장시 mIfno로 이름을 변경한다.
	 두번째 파라미터
	  	: BindingResult객체는 validator를 통해 폼 값을 검증한 후
	  	해당 결과를 전송받을 객체로, 폼 값에 문제가 있다면 해당 객체를 통해 확인
	  	할 수 있다.
	 세번째 파라미터
	  	: 데이터를 저장할 Model객체
	 */
	...
	public String registProc(
		...
		//폼 값 검증 완료 시 이동할 페이지의 경로
		String viewPage = "03Validate/memberDone";
		
		/*
		 유효성 검증을 위해 정의한 클래스의 객체를 생성한 후 전송된 폼 값을
		 저장한 커맨드 객체를 통해 폼 값의 유효성 체크를 실시한다. 매개변수로
		 유효성 체크를 할 커맨드 객체와 바인딩 결과(검증 결과)를 저장할 객체를
		 인수로 전달한다.
		 */
		...
		if(result.hasErrors()) {
			...
			//Model 객체에 에러 메세지를 저장한 후 
			model.addAttribute("formError", "폼 값 유효성 체크에 실패했습니다.");
			//가입 페이지로 다시 이동한다.
